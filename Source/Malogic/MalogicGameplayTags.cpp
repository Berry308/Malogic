...
namespace MalogicGameplayTags
{
	...
	UE_DEFINE_GAMEPLAY_TAG_COMMENT(GameplayCue_UI_Damage, "GameplayCue.UI.Damage", "GameplayCue used to present locally relevant damage feedback.");
	UE_DEFINE_GAMEPLAY_TAG_COMMENT(Message_Combat_Damage, "Message.Combat.Damage", "Message channel for locally presented damage feedback.");
	...
	UE_DEFINE_GAMEPLAY_TAG_COMMENT(Movement_Mode_Custom, "Movement.Mode.Custom", "This is invalid and should be replaced with custom tags.  See MalogicGameplayTags::CustomMovementModeTagMap.");
	...
	FGameplayTag FindTagByString(const FString& TagString, bool bMatchPartialString)
	{
		...
		if (!Tag.IsValid() && bMatchPartialString)
		{
			...
			for (const FGameplayTag& TestTag : AllTags)
			{
				...
				{
					UE_LOG(LogMalogic, Display, TEXT("Could not find exact match for tag [%s] but found partial match on tag [%s]."), *TagString, *TestTag.ToString());
					Tag = TestTag;
					break;
				}
			}
