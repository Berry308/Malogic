...
UViewModelService* UVMWorldManager::FindService(FName ServiceName) const
{
	if (const TObjectPtr<UViewModelService>* FoundService = Services.Find(ServiceName))
	{
		...
		UE_LOG(LogUI, Warning, TEXT("World ViewModel manager [%s] has an invalid Service registered under name [%s]."), *GetNameSafe(this), *ServiceName.ToString());
		return nullptr;
	}

	UE_LOG(LogUI, Warning, TEXT("World ViewModel manager [%s] could not find Service [%s]."), *GetNameSafe(this), *ServiceName.ToString());
	return nullptr;
}
